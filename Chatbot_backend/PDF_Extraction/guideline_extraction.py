...
PDF_NAME = "Guidelines 2006 goods & work.pdf"
...
OUT_JSON = BASE_DIR / "guidelines_2006.json"
...
def strip_headers_footers(page_text: str) -> str:
    ...
    for ln in page_text.splitlines():
        t = ln.strip()
        if re.fullmatch(r"(?:Page\s*)?\d{1,4}", t):
            continue  # lone page numbers
        if t in {"i", "ii", "iii", "iv", "v", "vi", "vii", "viii", "ix", "x"}:
            continue
        kept.append(ln)
    return "\n".join(kept)
...
CHAPTER_LINE_RE = re.compile(
    r"^\s*CHAPTER\s+(?P<chap>(\d+|[ivxlc]+))\b.*$", re.IGNORECASE | re.MULTILINE
)
...
def find_chapters(text: str):
    ...
    for i, m in enumerate(hits):
        ...
        chap_num_for_sections = (
            str(int(chap_token))
            if chap_token.isdigit()
            else str(roman_to_int(chap_token))
        )
...
if __name__ == "__main__":
    ...
    chapters = []
    for start, end, chap_token, chap_title, chap_num_for_sections in find_chapters(
        full_text
    ):
